package ivk.skill.api.infrastructure
...
open class ApiClient(val baseUrl: String, val client: Call.Factory = defaultClient) {
    companion object {
        ...
        const val baseUrlKey: String = "ivk.skill.api.baseUrl"
    ...
    protected fun <T> updateAuthParams(requestConfig: RequestConfig<T>) {
        if (requestConfig.headers["x-ivk-apikey"].isNullOrEmpty()) {
            if (apiKey["x-ivk-apikey"] != null) {
                if (apiKeyPrefix["x-ivk-apikey"] != null) {
                    requestConfig.headers["x-ivk-apikey"] = apiKeyPrefix["x-ivk-apikey"]!! + " " + apiKey["x-ivk-apikey"]!!
                } else {
                    requestConfig.headers["x-ivk-apikey"] = apiKey["x-ivk-apikey"]!!
                }
            }
        }
    }
